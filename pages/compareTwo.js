...
const Filter = () => {
    ...
    const drawChart = (data, order) => {
        ...
        svg.selectAll("rect")
            ...
            .enter().append("rect")
            .attr("x", d => x(d.codon))
            .attr("y", d => y(d.Gene) + (y.bandwidth() - ((y.bandwidth() / d.totalCount) * parseInt(d.count) * 10)) / 2) // Center vertically
            .attr("width", x.bandwidth())
            .attr("height", d => Math.max((y.bandwidth() / d.totalCount) * parseInt(d.count) * 10, 3)) // Set a minimum height of 1
